...
import com.netease.nim.uikit.common.ui.dialog.DeleteAndTopDialog;
...
//            showLongClickMenu(adapter.getItem(position), position);
            showDeleteOrTopDialog(adapter.getItem(position), position);
        }
    ...
    private void showDeleteOrTopDialog(final RecentContact recent, final int position) {
        String topShowContent = (isTagSet(recent, RECENT_TAG_STICKY)) ? "取消置顶" : "置顶聊天";

        DeleteAndTopDialog dialog = new DeleteAndTopDialog(getActivity(),topShowContent, new DeleteAndTopDialog.DeleteOrTopSelectListener() {
            @Override
    ...
    private void refreshMessages(boolean unreadChanged) {
        ...
        if (unreadChanged) {
            ...
            if (mUnReadMsgListener != null)
                mUnReadMsgListener.unreadNumb(unreadNum);
        }
    }
    ...
    private UnreadMsgImListener mUnReadMsgListener;

    public interface UnreadMsgImListener {
        void unreadNumb(int numb);
    }

    public void setUnReadMsgListener(UnreadMsgImListener listener) {
        mUnReadMsgListener = listener;
    }
}
