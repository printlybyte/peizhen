...
import com.netease.nim.uikit.business.session.actions.ImageTakeGalleryAction;
import com.netease.nim.uikit.business.session.actions.ImageTakePhotoAction;
...
public class MessageFragment extends TFragment implements ModuleProxy {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            if (requestCode == 0x110) {
                double latitude = data.getDoubleExtra(LocationActivity.KEY_LATITUDE, -1d);
                double longitude = data.getDoubleExtra(LocationActivity.KEY_LONGITUDE, -1d);
                String address = data.getStringExtra(LocationActivity.KEY_ADDRESS);
                if (latitude!=-1d){
                    IMMessage message = MessageBuilder.createLocationMessage(sessionId, sessionType, latitude, longitude,
                            address);
                    sendMessage(message);
                }
                return;
    ...
    protected List<BaseAction> getActionList() {
        ...
        actions.add(new ImageTakePhotoAction());
        actions.add(new ImageTakeGalleryAction());
        actions.add(new LocationAction(this));
